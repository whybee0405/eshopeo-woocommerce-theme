...
<li <?php wc_product_class( 'product-card', $product ); ?>>
	...
	<div class="card-media">
		...
		<button
			...
			aria-pressed="false"
			aria-label="<?php echo esc_attr( sprintf( /* translators: %s: product name. */ __( 'Save %s to wishlist', 'glow-glow' ), $product->get_name() ) ); ?>"
		>
	...
	<div class="card-body">
		<p class="card-meta mono">
			...
			<?php endif; ?>
			<?php if ( $glow_step_no ) : ?>
				<span class="card-step"><?php echo esc_html( sprintf( /* translators: %s: routine step number. */ __( 'STEP %s', 'glow-glow' ), str_pad( (string) (int) $glow_step_no, 2, '0', STR_PAD_LEFT ) ) ); ?></span>
			<?php endif; ?>
		</p>
