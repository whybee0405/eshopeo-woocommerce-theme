...
function glow_customizer_register( $wp_customize ) {
	...
	$notice_defaults = array(
		'glow_notice_1' => __( 'Free shipping over R500', 'glow-glow' ),
		'glow_notice_2' => __( 'Every batch verified', 'glow-glow' ),
		'glow_notice_3' => __( 'Authentic K-beauty', 'glow-glow' ),
	);
	...
	foreach ( $notice_defaults as $key => $default ) {
		...
		$wp_customize->add_control(
			$key,
			array(
				'label'   => sprintf( __( 'Notice item %d', 'glow-glow' ), (int) substr( $key, -1 ) ),
	...
	$wp_customize->add_section(
		'glow_trust',
		array(
			'title'    => __( 'Trust Signals', 'glow-glow' ),
			'panel'    => 'glow_panel',
			'priority' => 20,
		)
	);
	...
	$trust_defaults = array(
		'glow_trust_1' => __( 'Batch-verified imports', 'glow-glow' ),
		'glow_trust_2' => __( 'Free shipping over R500', 'glow-glow' ),
		'glow_trust_3' => __( 'Ships from Joburg', 'glow-glow' ),
	);
	...
	foreach ( $trust_defaults as $key => $default ) {
		...
		$wp_customize->add_control(
			$key,
			array(
				'label'   => sprintf( __( 'Trust signal %d', 'glow-glow' ), (int) substr( $key, -1 ) ),
				'section' => 'glow_trust',
				'type'    => 'text',
			)
		);
	...
	$wp_customize->add_section(
		'glow_contact',
		array(
			'title'    => __( 'Contact Details', 'glow-glow' ),
			'panel'    => 'glow_panel',
			'priority' => 30,
		)
	);
	...
	$contact_fields = array(
		'glow_whatsapp_number' => array(
			'label'   => __( 'WhatsApp number (display)', 'glow-glow' ),
			'default' => '+27 11 000 0000',
		),
		'glow_whatsapp_url' => array(
			'label'   => __( 'WhatsApp URL (wa.me link)', 'glow-glow' ),
			'default' => 'https://wa.me/27110000000',
		),
		'glow_email' => array(
			'label'   => __( 'Main email address', 'glow-glow' ),
			'default' => 'hello@glowglow.co.za',
		),
		'glow_email_korean' => array(
			'label'   => __( 'Korean enquiries email', 'glow-glow' ),
			'default' => 'korean@glowglow.co.za',
		),
		'glow_hours' => array(
			'label'   => __( 'Business hours', 'glow-glow' ),
