...
function glow_faq_items() {
	return array(
		array(
			'question' => __( 'How do I know your products are authentic?', 'glow-glow' ),
			'answer'   => __( 'Every product we sell is sourced directly from the brand or its licensed Korean distributor — never from grey-market consolidators. Each shipment arrives with batch documentation, and we keep the batch number of every unit we sell on file. If you ever want the paperwork for something you bought, email us the order number and we will send it.', 'glow-glow' ),
		),
		array(
			'question' => __( 'How do I find fragrance-free products?', 'glow-glow' ),
			'answer'   => __( 'Use the Sensitive skin-type filter on any shop page — everything tagged sensitive is either fragrance-free or uses only trace functional fragrance, and the product page says which. Every product also lists its key actives up front, so you can rule things out before you click.', 'glow-glow' ),
		),
		array(
			'question' => __( 'How should I patch test a new product?', 'glow-glow' ),
			'answer'   => __( 'Apply a small amount to the inside of your forearm or behind your ear once a day for three days. If nothing flares, try it on a small area of your face for another two or three nights before using it fully. Introduce one new product at a time — if two new things go on together and your skin reacts, you will not know which one did it.', 'glow-glow' ),
		),
		array(
			'question' => __( 'How long does delivery take?', 'glow-glow' ),
			'answer'   => __( 'Orders ship from Johannesburg within one working day. Gauteng usually receives in 1–3 working days, Western Cape and KwaZulu-Natal in 2–4, and regional or outlying areas in 3–5. You will get a tracking number the moment your parcel is collected. Shipping is free over R500.', 'glow-glow' ),
		),
		array(
			'question' => __( 'What is your returns policy if my skin reacts?', 'glow-glow' ),
		...
		array(
			'question' => __( 'What order do I apply products in?', 'glow-glow' ),
			'answer'   => __( 'Thinnest to thickest, water before oil: cleanse, exfoliate (2–3 times a week, not daily), tone, treat with serums or ampoules, moisturise, eye cream, and SPF every morning — including cloudy Joburg winter mornings. Our whole shop is organised in this order, so if you shop the steps left to right you cannot get the sequence wrong.', 'glow-glow' ),
		),
		array(
			'question' => __( 'Do you ship outside South Africa?', 'glow-glow' ),
			'answer'   => __( 'Not yet. We currently deliver anywhere in South Africa via tracked courier. Namibia and Botswana are on the roadmap — join the newsletter and you will hear about it the month it happens.', 'glow-glow' ),
		),
	);
}
...
function glow_meta_description() {
	...
	if ( is_front_page() ) {
		return __( 'Build your Korean skincare routine in the right order — cleanse to SPF, seven steps, zero guesswork. Authentic K-beauty, batch-verified and shipped across South Africa.', 'glow-glow' );
	}
	...
	if ( glow_wc_active() && is_product() ) {
		...
		if ( $product ) {
			...
			if ( $actives ) {
				...
				$parts[] = sprintf( __( 'Key actives: %s.', 'glow-glow' ), $actives );
			}
			if ( $excerpt ) {
				$parts[] = $excerpt;
			}
			$parts[] = __( 'Authentic, batch-verified, delivered across South Africa.', 'glow-glow' );
	...
	if ( ( glow_wc_active() && ( is_shop() || is_product_taxonomy() ) ) || is_tax( 'skin_concern' ) || is_tax( 'skin_type' ) ) {
		...
		$name = ( $term && isset( $term->name ) ) ? $term->name : __( 'K-beauty', 'glow-glow' );
		return sprintf(
			/* translators: %s: category or term name. */
			__( 'Shop %s without the noise — short, honest descriptions, full ingredient lists and fast filtering by skin type and concern. Free delivery over R500 in South Africa.', 'glow-glow' ),
			$name
		);
	}
	...
	if ( is_page_template( 'page-about.php' ) ) {
		return __( 'A Johannesburg team with direct Seoul relationships. Every batch verified, every ingredient listed — K-beauty you can buy for yourself or someone you love without second-guessing it.', 'glow-glow' );
	}

	// Persona 5 — The Sensitive-Skin Sceptic.
	if ( is_page_template( 'page-faq.php' ) ) {
		return __( 'Fragrance-free filtering, patch-test guidance, a returns policy that covers skin reactions, and delivery times by province. The questions sensitive skin actually asks, answered plainly.', 'glow-glow' );
	}
	...
	if ( is_search() ) {
		return sprintf(
			...
			__( 'Search results for “%s” — authentic K-beauty, organised by routine step, shipped across South Africa.', 'glow-glow' ),
			get_search_query()
		);
	}
	...
	return __( 'Authentic Korean skincare organised by the 7-step routine, batch-verified and shipped across South Africa.', 'glow-glow' );
}
	...
	if ( glow_wc_active() && is_product() ) {
		...
		if ( $product ) {
			...
			$crumbs = array(
				...
				array( __( 'Shop', 'glow-glow' ), wc_get_page_permalink( 'shop' ) ),
			);
