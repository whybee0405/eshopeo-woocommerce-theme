		...
		<div class="container">
			...
			<div class="pdp-grid">
				...
				<div class="pdp-gallery">
					...
					<?php if ( $glow_gallery_ids ) : ?>
						<div class="pdp-thumbs" role="group" aria-label="<?php esc_attr_e( 'Product images', 'glow-glow' ); ?>">
				...
				<div class="pdp-info">
					...
					<div class="pdp-rating-row">
						<?php if ( $product->get_review_count() > 0 ) : ?>
							...
							<a class="mono" href="#reviews"><?php echo esc_html( sprintf( /* translators: %s: review count. */ _n( '%s review', '%s reviews', $product->get_review_count(), 'glow-glow' ), number_format_i18n( $product->get_review_count() ) ) ); ?></a>
						<?php endif; ?>
					...
					<?php if ( $glow_vegan || $glow_cf ) : ?>
						<p class="pdp-marks mono">
							<?php if ( $glow_vegan ) : ?><span>✓ <?php esc_html_e( 'Vegan', 'glow-glow' ); ?></span><?php endif; ?>
							<?php if ( $glow_cf ) : ?><span>✓ <?php esc_html_e( 'Cruelty-free', 'glow-glow' ); ?></span><?php endif; ?>
						</p>
					<?php endif; ?>
			...
			<div class="pdp-accordions">
				...
				<details class="faq-item" open>
					<summary><?php esc_html_e( 'About this product', 'glow-glow' ); ?></summary>
					<div class="faq-body prose">
						<?php
						$glow_description = $product->get_description();
						echo $glow_description ? wp_kses_post( wpautop( $glow_description ) ) : '<p>' . esc_html__( 'Details are on their way for this one — the actives and skin-fit chips above are the short version.', 'glow-glow' ) . '</p>';
						?>
					</div>
				</details>
				...
				<?php if ( $glow_step ) : ?>
					<details class="faq-item">
						<summary><?php esc_html_e( 'How it fits your routine', 'glow-glow' ); ?></summary>
						<div class="faq-body">
							<p>
								<?php
								echo esc_html(
									sprintf(
										...
										__( 'This is a step %1$s product — %2$s. Apply it in this position, thinnest texture to thickest:', 'glow-glow' ),
										(int) $glow_step_no,
										$glow_step['name']
									)
								);
								...
								<?php foreach ( $glow_steps as $glow_i => $glow_s ) : ?>
									<li class="<?php echo ( $glow_i + 1 ) === $glow_step_no ? 'is-current' : ''; ?>">
										...
										<?php if ( ( $glow_i + 1 ) === $glow_step_no ) : ?>
											<span class="mono you-are-here"><?php esc_html_e( '← this product', 'glow-glow' ); ?></span>
										<?php endif; ?>
									</li>
								<?php endforeach; ?>
							</ol>
				...
				<details class="faq-item">
					<summary><?php esc_html_e( 'Full ingredient disclosure', 'glow-glow' ); ?></summary>
					<div class="faq-body">
						<?php if ( $glow_actives ) : ?>
							<p><strong><?php esc_html_e( 'Key actives:', 'glow-glow' ); ?></strong> <?php echo esc_html( $glow_actives ); ?>.</p>
						<?php endif; ?>
						<p><?php esc_html_e( 'The complete INCI list is printed on the carton and inner label of every unit, exactly as registered in Korea — we don\'t relabel or repackage. Want the list before you buy, or the documentation for a specific batch? Email us the product name and we\'ll send both.', 'glow-glow' ); ?></p>
						<p><?php esc_html_e( 'New to this product? Patch test first: inside of the forearm, once a day for three days, before it goes near your face.', 'glow-glow' ); ?></p>
					</div>
				</details>
				...
				<details class="faq-item" id="reviews">
					<summary>
						...
						echo esc_html(
							$glow_rc > 0
								? sprintf( /* translators: %s: review count. */ _n( 'Reviews (%s)', 'Reviews (%s)', $glow_rc, 'glow-glow' ), number_format_i18n( $glow_rc ) )
								: __( 'Reviews', 'glow-glow' )
						);
						?>
					</summary>
					<div class="faq-body">
			...
			<?php if ( $glow_next_products ) : ?>
				<section class="section section-tight pdp-continue">
					<div class="section-head">
						<div>
							<p class="eyebrow"><?php esc_html_e( 'What comes next', 'glow-glow' ); ?></p>
							<h2 class="t-1"><?php esc_html_e( 'Continue the routine', 'glow-glow' ); ?></h2>
						</div>
					</div>
