...
$glow_title = $glow_term && isset( $glow_term->name ) ? $glow_term->name : __( 'All products', 'glow-glow' );
$glow_intro = '';

if ( is_search() ) {
	/* translators: %s: search query. */
	$glow_title = sprintf( __( 'Results for “%s”', 'glow-glow' ), get_search_query() );
	$glow_intro = __( 'We search product names, descriptions and key actives.', 'glow-glow' );
...
<main id="main">
	...
	<div class="container shop-layout">
		...
		<aside class="shop-filters" data-filter-drawer aria-label="<?php esc_attr_e( 'Product filters', 'glow-glow' ); ?>">
			<div class="filters-head">
				<h2 class="mono"><?php esc_html_e( 'Refine', 'glow-glow' ); ?></h2>
				<button class="filters-close" type="button" aria-label="<?php esc_attr_e( 'Close filters', 'glow-glow' ); ?>" data-filters-close>
			...
			<div class="filter-group">
				<h3 class="filter-label mono"><?php esc_html_e( 'By step', 'glow-glow' ); ?></h3>
			...
			$glow_filter_taxes = array(
				'skin_type'    => __( 'Skin type', 'glow-glow' ),
				'skin_concern' => __( 'Concern', 'glow-glow' ),
			);
			...
			<?php if ( is_shop() || is_product_taxonomy() ) : ?>
				<div class="filter-group">
					<h3 class="filter-label mono"><?php esc_html_e( 'Price', 'glow-glow' ); ?></h3>
		...
		<div class="shop-main">
			<div class="shop-toolbar">
				<button class="btn btn-outline filter-toggle" type="button" data-filter-toggle aria-expanded="false">
					...
					<?php esc_html_e( 'Filter', 'glow-glow' ); ?>
				</button>
				...
				<p class="result-count mono">
					...
					echo esc_html(
						sprintf(
							...
							_n( '%s product', '%s products', $glow_total, 'glow-glow' ),
			...
			<?php if ( have_posts() ) : ?>
				...
				<nav class="shop-pagination" aria-label="<?php esc_attr_e( 'Products pagination', 'glow-glow' ); ?>">
					<?php
					the_posts_pagination(
						array(
							'mid_size'  => 1,
							'prev_text' => __( '← Previous', 'glow-glow' ),
							'next_text' => __( 'Next →', 'glow-glow' ),
						)
					);
					?>
				</nav>
			...
			<?php else : ?>
				...
				<div class="shop-empty">
					<h2 class="t-2"><?php esc_html_e( 'Nothing matches that combination — yet.', 'glow-glow' ); ?></h2>
					<p><?php esc_html_e( 'Try widening the filters, or search by ingredient — “snail mucin”, “niacinamide” and “centella” all work.', 'glow-glow' ); ?></p>
					<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="search-form-row">
						<label class="screen-reader-text" for="glow-empty-search"><?php esc_html_e( 'Search products', 'glow-glow' ); ?></label>
						<input type="search" id="glow-empty-search" name="s" placeholder="<?php esc_attr_e( 'Search by ingredient', 'glow-glow' ); ?>" />
